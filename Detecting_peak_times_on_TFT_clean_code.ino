...
void setup(){//the entire code in this loop will only happen 1 time
  ...
  while(j<11){//the entire code within the while loop repeats until we get 11 values in peaktArr
    ...
    for(i = 2; i < 3; i++){//read and store 1 value in the 3rd column of pAndt 
      pAndt[0][i] = analogRead(A0);//1st row of pAndt stores the sensor values
      pAndt[1][i] = millis();// 2nd row of pAndt stores the time stamp of the sensor values
      while (millis() - delayTime  <=200){//delay in ms before reading the next value. we read 5 values every second
      }
    }
    for(i = 1; i < 2; i++){   
      if(pAndt[0][i]>pAndt[0][i-1] && pAndt[0][i]>pAndt[0][i+1]){ //compare the 3 values in pAndt to determine a peak
    ...
    while(xPos<=160){//this while loop is carried out as long as the current point of the waveform has not reached the edge of the screen
    int sensor = analogRead(A0); 
    int graphHeight = map(sensor,0,682,5,screen.height()/1.5);// 0 and 682 are the digital equivalent of the max and min anolog voltage from the sensor unit and are mapped to 5 and screen.height()/1.5 which are two different screen heights  
    ...
    xPos++;
    delayTime = millis();
    while(millis() - delayTime<=25){//rate at which points are displayed on screen to plot waveform
    }
}
}
void loop(){
